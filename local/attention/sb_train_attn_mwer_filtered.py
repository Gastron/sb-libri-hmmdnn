...
def dataio_prepare(hparams):
    ...
    other_uttids = []
    with open(hparams["other_uttids"]) as fi:
        for line in fi:
            uttid, *_ = line.strip().split(maxsplit=1)
            other_uttids.append(uttid)
    other_uttids = set(other_uttids)

    def filter_uttids(sample, uttids=other_uttids):
        return sample["__key__"] in uttids

    traindata = (
            wds.WebDataset(hparams["trainshards"])
            .select(filter_uttids)
            .decode()
